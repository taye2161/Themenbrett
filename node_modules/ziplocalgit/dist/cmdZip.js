...
const copilothistory_1 = require("copilothistory");
...
async function cmdZip(options) {
    ...
    const enforceRemote = options.enforceRemote;
    const noRemote = options.noRemote;
    const remoteType = options.remoteType;
    const remotePath = options.remotePath;
    if (!await (0, cmdfiles_1.folderExists)(REPO)) {
    ...
    try {
        ...
        if (options.addCopilotHistory) {
            try {
                const copilotHistoryFile = options.copilotHistoryFilename;
                if (await (0, cmdfiles_1.fileExists)(copilotHistoryFile)) {
                    await fs.rm(copilotHistoryFile);
                }
                const copilotHistory = await (0, copilothistory_1.exportCopilotHistory)({
                    output: copilotHistoryFile,
                    enforceRemote,
                    noRemote,
                    remoteType,
                    remotePath,
                ...
                if (await (0, cmdfiles_1.fileExists)(copilotHistoryFile)) {
                    zip.addLocalFile(copilotHistoryFile, '', copilotHistoryFile);
                }
            }
            catch (err) {
                (0, cmdfiles_1.verb)(`Die Copilot History konnte nicht exportiert werden: ${err}. Die Zip Datei wird trotzdem erstellt, aber ohne die Copilot History.`);
            }
        }
        ...
        if (options.addCopilotHistory && await (0, cmdfiles_1.fileExists)(options.copilotHistoryFilename)) {
            try {
                await fs.rm(options.copilotHistoryFilename);
            }
            catch (err) {
                (0, cmdfiles_1.verb)(`Die temporäre Copilot History Datei ${options.copilotHistoryFilename} konnte nicht gelöscht werden, bitte manuell überprüfen.`);
            }
        }
        try {
            const stats = await fs.stat(filename);
            const size = Math.round(stats.size / 1000);
            (0, cmdfiles_1.log)(`Die Abgabedatei "${filename}"(${size} KB) wurde erfolgreich erstellt.`);
            if (size > maxsize) {
                (0, cmdfiles_1.log)(`Achtung: Die Abgabedatei "${filename}" erscheint ungewöhnlich groß.\nFalls Du die Datei aufgrund der Größe nicht bei Moodle abgeben kannst, \nwende Dich per E - Mail an den Betreuer.\nSchicke aber auf keinen Fall die Datei per E - Mail!`);
            }
            else {
